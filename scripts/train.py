...
runs/detect/<name>/weights/best.pt — copy to weights/best.pt for the web app.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--data",
        default=str(ROOT / "data" / "dataset" / "dataset.yaml"),
        help="Path to dataset.yaml (created by scripts/prepare_dataset.py)",
    )
    parser.add_argument("--model", default="yolo11n.pt", help="Base model to fine-tune (yolo11n/s/m/l/x.pt)")
    ...
    parser.add_argument("--batch", type=int, default=8, help="Batch size per training step")
    parser.add_argument("--name", default="run1", help="Run name. Outputs land in runs/detect/<name>/")
    parser.add_argument(
        "--device",
        default=None,
        help="Force a device (e.g. 'cpu', 'mps', '0' for GPU 0). Default lets Ultralytics auto-detect.",
    )
